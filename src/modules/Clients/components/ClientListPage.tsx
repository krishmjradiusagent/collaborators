...
import { MOCK_CLIENTS, GLOBAL_COLLABORATOR_POOL, MOCK_ASSIGNMENTS } from "../mockData"
        ...
        <Table>
          ...
          <TableBody>
            {MOCK_CLIENTS.map((client) => {
              const assignments = MOCK_ASSIGNMENTS.filter(a => a.clientId === client.id)
                  ...
                  <TableCell>
                    <div className="flex -space-x-3 items-center">
                       {assignedCollabs.map((collab, i) => (
                         <Avatar key={i} className="h-9 w-9 border-4 border-white shadow-sm ring-1 ring-slate-100 hover:z-10 transition-transform hover:scale-125 cursor-pointer">
                            <AvatarFallback className="bg-slate-50 text-[10px] font-black text-[#5A5FF2]">{collab.name.split(' ').map(n=>n[0]).join('')}</AvatarFallback>
                         </Avatar>
                       ))}
                       <button className="h-9 w-9 rounded-full border-4 border-white bg-slate-50 flex items-center justify-center text-slate-300 hover:bg-[#5A5FF2] hover:text-white transition-all shadow-sm hover:scale-110 hover:z-10">
